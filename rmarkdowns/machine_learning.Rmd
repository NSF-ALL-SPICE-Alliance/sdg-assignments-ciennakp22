...
```
```{r}
importance_df_fw <- as.data.frame(rf_freshwat$importance)
```
```{r}
importance_df_fw_top_10 <- importance_df_fw %>%
  rownames_to_column(var = "variable") %>%
  slice_max(n = 10, order_by = `%IncMSE`)
```
```{r}
ggplot(importance_df_fw_top_10, aes(x = `%IncMSE`, y = reorder(variable, `%IncMSE`))) +
  geom_bar(stat = "identity", fill = "coral", color = "black") +
  theme_minimal() +
  labs(title = "Most Important Variables in Predicting Freshwater Ecosystem Change",
       subtitle = "Top 10",
       y = "SDG Indicator",
       x = "Feature Importance (% Increase in Mean Squared Error)")
```
