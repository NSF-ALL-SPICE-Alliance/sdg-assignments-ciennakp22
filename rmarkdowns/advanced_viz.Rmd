...
goal_3_4_scatter <- ggplot(data = sdr_data, aes(x= goal_3_score, y = goal_4_score, color = regions_used_for_the_sdr)) +
  ...
  theme_minimal() +
  scale_fill_viridis_d()+
  labs(title = "Relationship Between SDG 3 and SDG 4 Scores by Region",
       x = "SDG 3 Score",
       y = "SDG 4 Score",
       color = "Region")
...
mytext <- paste(
    "Country: ", sdr_data_world_joined$country,"<br/>", 
    "Goal 10 Score: ", round(sdr_data_world_joined$goal_7_score, 2), 
    sep="") %>%
  lapply(htmltools::HTML)
...
leaflet(sdr_data_world_joined) %>% 
  addTiles()  %>% 
  setView( lat=10, lng=0 , zoom=2) %>%
  addPolygons(stroke = FALSE, fillOpacity = 0.5, smoothFactor = 0.5, color = ~colorQuantile("Dark2", goal_10_score)(goal_10_score), label = mytext)
```
