...
```{r, warning=FALSE, message=FALSE}
ggplot(sdr_data, aes(x = goal_2_score, 
                     y = goal_11_score)) +
  geom_point() +
  geom_smooth() +
  stat_cor() +
...
```{r, warning=FALSE, message=FALSE}
goal_2_11_scatter <- ggplot(sdr_data, aes(x = goal_2_score, 
...
ggplotly(goal_2_11_scatter)

```

It looks like there's a positive correlation between SDG 2 (food security and nutrition) and SDG 11 (sustainable cities and communities), meaning countries that do well in food security also tend to have better urban sustainability. One possible reason is that strong food systems and nutrition programs might go hand in hand with better city planning and infrastructure, making communities more resilient overall.

But there are a few outliers that don’t quite follow this pattern. For example, Brunei Darussalam has a really high SDG 11 score but a lower SDG 2 score, which could mean it has well-developed cities but still faces some challenges with food security. These outliers could be due to unique policies, economic conditions, or other factors that affect each country differently.
