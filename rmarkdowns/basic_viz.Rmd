...
```{r}
ggplot(data = sdr_data, aes(x= goal_7_score, y = goal_9_score, color = goal_16_score)) +
  geom_point()
```

Interpretation: This scatter plot shows that as goal_7_score increases (x-axis), goal_9_score (y-axis) also tends to increase. The lighter blue points, representing higher goal_16_score, are mostly found in the upper-right area, where both scores are high. Darker blue points, indicating lower goal_16_score, are spread more across the lower and middle ranges of the graph.
...
ggplot(sdr_data, aes(x = goal_9_score, fill=regions_used_for_the_sdr)) + 
  geom_histogram(bins=33, color="black")
```
Interesting: This histogram shows the distribution of goal_9_score across regions. Unlike the previous plot, the scores are more evenly spread across the range, with no single region dominating the highest or lowest scores. OECD countries (pink) appear frequently across the middle to higher ranges, while Sub-Saharan Africa (gray) is visible across lower and mid-range scores. Other regions, such as East and South Asia (green) and Europe and Central Asia (orange), are distributed throughout the range without a clear concentration in any specific area.